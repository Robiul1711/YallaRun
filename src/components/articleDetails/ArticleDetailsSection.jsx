...
// const ArticleDetailsSection = ({id}) => {
//   const axiosPublic = useAxiosPublic();

//   const {data: article, isLoading} = useQuery({
//     queryKey: ["article-details", id],
//     queryFn: async () => {
//       const res = await axiosPublic.get(`/article/show/${id}`)
//       return res.data
//     }
//   })
...
const ArticleDetailsSection = ({ id }) => {
  ...
  // API returns {data: {...}} so we destructure for convenience
        ...
        <div className="flex sm:flex-row flex-col sm:items-center xlg:gap-10 gap-4 sm:mb-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex items-center gap-3">
              <div>
                ...
                {/* Example date—replace with createdAt if API returns it */}
                <div className="text-sm text-gray-600">
                  {new Date().toLocaleDateString()}
                </div>
              </div>
              <button className="px-4 py-1 border border-secondaryColor text-secondaryColor text-sm rounded-full hover:bg-secondaryColor hover:text-white duration-200">
                Follow
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-gray-500">
            <div className="flex items-center gap-3">
              ...
              <Bookmark size={18} className="cursor-pointer hover:text-yellow-500" />
            </div>
          </div>
        </div>
      </header>
      ...
      <article className="prose prose-lg max-w-none">
        ...
        <section className="mt-8">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: art.conclusion_des }}
          />
        </section>
      </article>
    </div>
